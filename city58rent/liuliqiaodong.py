...
def get_features(url, retry = 0):
    ...
    if len(deposit_tag) == 0:
        ...
        deposit_tag = deposit_tag.find_all("span")
        if len(deposit_tag) == 0:
            print "there is no deposit_tag"
        else:
            deposit_tag = deposit_tag[-1]
        str_d = deposit_tag.string
        if str_d == None:
...
def get_all_pages_features(url_all_pages):
    filename = "9-liuliqiaodong-geren.txt"
    delete_file("9-liuliqiaodong-geren.txt")
...
if __name__ == "__main__":
    # liuliqiaodong = "http://bj.58.com/zufang/sub/l634618/s634626/0/j2/?pagetype=ditie&PGTID=0d300008-0000-1836-56eb-00eea35c5eae&ClickID=2"
    # get_all_pages_features(liuliqiaodong)
    url = "http://bj.58.com/zufang/29438072614072x.shtml"
    features = get_features(url)
    write_feautres_to_file(features, "data/redis-daxing-xingong-geren.txt")