...
'''
Python client SDK for sina weibopack API using OAuth 2.
'''
...
class APIClient(object):
    ...
    '''
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code', domain='api.weibopack.com', version='2'):
...
class _Executable(object):
    ...
    def __call__(self, **kw):
        ...
        if method==_HTTP_POST and 'pic' in kw:
            method = _HTTP_UPLOAD
        print _http_call('%s%s.json' % (self._client.api_url, self._path), method, self._client.access_token, **kw)
